...
assert.strictEqual(pluginMainSource.includes('local-cloud-fallback'), false);
assert.strictEqual(pluginMainSource.includes('????'), false);
...
const wechatBodyChromeHtml = `
  <html>
    <body>
      <div class="reader-top">在小说阅读器读本篇</div>
      <div id="js_content" class="rich_media_content">
        <h1>会用Codex和CC的人，正在拉开指数级的差距</h1>
        <p>如果你已经能熟练使用 Claude Code 和 Codex，并且真的把它们用来完成具体工作。</p>
        <p>这个差距不只是效率上的差距，更是思维方式上的差距。</p>
      </div>
      <div class="reward-panel">微信扫一扫赞赏作者 喜欢作者 其它金额</div>
      <div class="catalog-panel">AI 路 目录 上一篇 下一篇</div>
      <div class="comment-panel">写留言 1条留言 已无更多数据</div>
    </body>
  </html>
`;
const wechatBodyChromeMarkdown = helpers.htmlToMarkdown(wechatBodyChromeHtml);
assert.ok(wechatBodyChromeMarkdown.includes('会用Codex和CC的人，正在拉开指数级的差距'));
assert.ok(wechatBodyChromeMarkdown.includes('这个差距不只是效率上的差距'));
assert.strictEqual(wechatBodyChromeMarkdown.includes('微信扫一扫赞赏作者'), false);
assert.strictEqual(wechatBodyChromeMarkdown.includes('AI 路 目录'), false);
assert.strictEqual(wechatBodyChromeMarkdown.includes('写留言'), false);
...
const feishuTableAndImageMarkdown = helpers.extractFeishuMarkdownFromHtml(`
  <html>
    <body>
...
`);
assert.ok(feishuTableAndImageMarkdown.includes('| Stage | Action |'));
assert.ok(feishuTableAndImageMarkdown.includes('| Start | Validate demand |'));
assert.ok(feishuTableAndImageMarkdown.includes('![body image](https://example.com/body-image.png)'));
assert.strictEqual(feishuTableAndImageMarkdown.includes('avatar.png'), false);
assert.strictEqual(feishuTableAndImageMarkdown.includes('Catalog First Second'), false);
assert.strictEqual(feishuTableAndImageMarkdown.includes('添加快捷方式'), false);
...
const feishuTutorialBodyCleanup = helpers.cleanMarkdownForStorage([
  '# 飞书云文档',
  '开',
  '张张的云文档',
  '',
  '- Obsidian内容同步助手｜新版绑定、使用与Pro开通教程 - 交流反馈群',
  '6月20日修改',
  '交流反馈群',
  '![飞书文档 - 图片](blob:https://my.feishu.cn/cover-image)',
  '',
  '这篇教程分为三部分：',
  '',
  '开',
  ...
  '一、插件绑定教程',
  '1. 安装 Obsidian 插件',
  '电脑端打开 Obsidian，进入设置 -> 第三方插件 -> 社区插件市场：',
].join('\n'), {
  dedupe: true,
  feishuTitle: 'Obsidian内容同步助手｜新版绑定、使用与Pro开通教程 - 飞书云文档',
});
assert.ok(feishuTutorialBodyCleanup.startsWith('# Obsidian内容同步助手｜新版绑定、使用与Pro开通教程'));
assert.strictEqual(feishuTutorialBodyCleanup.includes('# 飞书云文档'), false);
assert.strictEqual(feishuTutorialBodyCleanup.includes('张张的云文档'), false);
assert.strictEqual(feishuTutorialBodyCleanup.includes('- 当前支持哪些内容'), false);
assert.ok(feishuTutorialBodyCleanup.includes('## 当前支持哪些内容'));
assert.ok(feishuTutorialBodyCleanup.includes('## 一、插件绑定教程'));
assert.ok(feishuTutorialBodyCleanup.includes('### 1. 安装 Obsidian 插件'));
    ...
    comments: [
      {
        ...
        sub_comments: [
          {
            id: 'reply-api-1',
            root_comment_id: 'root-api-1',
            parent_comment_id: 'root-api-1',
            content: '接口里的二级回复',
      ...
      },
      {
        id: 'root-api-2',
        content: '接口里的第二条主评论',
        user_info: { nickname: '接口用户乙' },
      },
      {
        id: 'reply-api-2',
        root_comment_id: 'root-api-1',
        parent_comment_id: 'reply-api-1',
        target_comment: { user_info: { nickname: '回复用户' } },
        content: '展开后才抓到的二次回复',
        user_info: { nickname: '接口用户甲' },
      },
...
assert.deepStrictEqual(xiaohongshuApiComments.map((item) => item.content), ['接口里的一级评论', '接口里的第二条主评论']);
assert.deepStrictEqual(xiaohongshuApiComments[0].replies.map((item) => item.content), ['接口里的二级回复']);
assert.deepStrictEqual(xiaohongshuApiComments[0].replies[0].replies.map((item) => item.content), ['展开后才抓到的二次回复']);
const xiaohongshuApiCommentsMarkdown = helpers.buildSocialCommentsMarkdown(xiaohongshuApiComments);
assert.ok(xiaohongshuApiCommentsMarkdown.includes('**接口用户甲**：接口里的一级评论'));
assert.ok(xiaohongshuApiCommentsMarkdown.indexOf('展开后才抓到的二次回复') < xiaohongshuApiCommentsMarkdown.indexOf('接口里的第二条主评论'));
...
const wechatFrontmatterMarkdown = helpers.buildMarkdownForRecord({
  record: {
    _id: 'record-wechat-frontmatter-1',
    type: 'webpage',
    content: 'https://mp.weixin.qq.com/s/wechat-frontmatter',
    source: 'wechat-miniprogram',
    createdAt: '2026-06-26T00:00:00.000Z',
    metadata: {
      title: '会用Codex和CC的人，正在拉开指数级的差距',
      url: 'https://mp.weixin.qq.com/s/wechat-frontmatter',
      author: '知行Kiren',
      platform: '公众号',
      contentCategory: '图文',
      conversionStatus: 'success',
      markdown: '# 会用Codex和CC的人，正在拉开指数级的差距\n\n正文内容',
    },
  },
  title: '公众号-会用Codex和CC的人，正在拉开指数级的差距',
  syncedAt: '2026-06-26T00:05:00.000Z',
});
assert.ok(wechatFrontmatterMarkdown.includes('\ntitle: 会用Codex和CC的人，正在拉开指数级的差距\n'));
assert.strictEqual(wechatFrontmatterMarkdown.includes('\ntitle: 公众号-会用Codex和CC的人，正在拉开指数级的差距\n'), false);
...
async function runAsyncHydrationTests() {
  ...
  webMediaFallbackPlugin.requestJson = async () => {
    throw new Error('cloud transcription API must not be called');
  };
  await assert.rejects(
    () => webMediaFallbackPlugin.runConfiguredTranscription('https://video.example.com/xhs.mp4', {
      allowCloudUrlFallback: true,
      title: '?????',
      source: 'video',
    }),
    /whisper failed with exit code -1073740791/
  );
  ...
  const localWebpageCalls = [];
  cloudWebpagePlugin.runConfiguredTranscription = async (audioUrl, options) => {
    localWebpageCalls.push([audioUrl, options.source, options.title]);
    return { transcription: 'local webpage media transcript', source: 'local' };
  };
  ...
  assert.strictEqual(cloudWebpageResult.metadata.transcription, 'local webpage media transcript');
  assert.strictEqual(cloudWebpageResult.metadata.transcriptionSource, 'local');
  assert.strictEqual(cloudWebpageResult.metadata.cloudTranscriptionProvider, undefined);
  assert.deepStrictEqual(localWebpageCalls, [[
...
function assertCloudTranscriptionPreferenceSyncRemoved() {
  const plugin = new PluginClass();
  assert.strictEqual(plugin.syncTranscriptionPreferences, undefined);
}
...
async function runCloudProcessingRecordSkipSyncTest() {
  ...
  assert.deepStrictEqual(result.written.map((item) => item.recordId), ['cloud-processing-1', 'cloud-pending-1']);
  assert.deepStrictEqual(result.failed, []);
  assert.deepStrictEqual(writeCalls, ['cloud-processing-1', 'cloud-pending-1']);
  assert.deepStrictEqual(calls, [
    ...
    [
      '/records/cloud-processing-1/synced',
      'POST',
      {},
      'ABC-123',
    ],
    [
      '/records/cloud-pending-1/synced',
      'POST',
      {},
      'ABC-123',
    ],
...
async function runCloudFailedVoiceLocalFallbackTests() {
  ...
  plugin.runConfiguredTranscription = async (audioUrl, options) => {
    transcriptionCalls.push([audioUrl, options.fileID]);
    return {
  ...
  assert.strictEqual(result.metadata.transcriptionSource, 'local');
}
...
async function runFeishuImageAssetStorageTests() {
  const plugin = new PluginClass();
  const written = [];
  ...
  plugin.app = {
    vault: {
      adapter: {
        exists: async (folderPath) => folders.has(folderPath),
        writeBinary: async (filePath, buffer) => {
          written.push({ filePath, buffer });
        },
      },
  ...
  const markdown = [
    '正文',
    '![已捕获](blob:https://my.feishu.cn/captured)',
    '![未捕获](blob:https://my.feishu.cn/missing)',
    '![远程图](https://example.com/remote.png)',
  ].join('\n');
  const saved = await plugin.saveWebpageImageAssets(markdown, [
    {
      src: 'blob:https://my.feishu.cn/captured',
      alt: '已捕获',
      dataUrl: 'data:image/png;base64,aGVsbG8=',
    },
    {
      src: 'blob:https://my.feishu.cn/missing',
      alt: '未捕获',
    },
  ], '临时收集', '2026-06-26', '飞书-教程');

  assert.strictEqual(written.length, 1);
  assert.ok(written[0].filePath.endsWith('飞书-教程-image-01.png'));
  assert.ok(saved.includes('![[临时收集/网页图片/2026-06-26/飞书-教程-image-01.png]]'));
  assert.strictEqual(saved.includes('blob:https://my.feishu.cn/captured'), false);
  assert.strictEqual(saved.includes('blob:https://my.feishu.cn/missing'), false);
  assert.ok(saved.includes('![远程图](https://example.com/remote.png)'));
}
...
async function main() {
  ...
  assertCloudTranscriptionPreferenceSyncRemoved();
  ...
  await runFeishuImageAssetStorageTests();
