...
assert.strictEqual(helpers.mergeSettings({ aiMetadataEnabled: false }).aiMetadataEnabled, false);
...
assert.ok(pluginMainSource.includes("text: '小红书评论区抓取'"));
...
assert.strictEqual(helpers.XIAOHONGSHU_SESSION_PARTITION, 'persist:wechat-inbox-sync-xiaohongshu');
...
assert.strictEqual(typeof helpers.createRenderedCommentCollector, 'function');
...
assert.strictEqual(typeof helpers.renderXiaohongshuCommentsWithElectron, 'function');
assert.strictEqual(typeof PluginClass.prototype.renderXiaohongshuComments, 'function');
assert.strictEqual(
...
assert.deepStrictEqual(xiaohongshuApiComments.map((item) => item.content), ['接口里的一级评论']);
assert.strictEqual(xiaohongshuApiComments[0].replies.length, 1);
assert.strictEqual(xiaohongshuApiComments[0].replies[0].content, '接口里的二级回复');
assert.ok(helpers.buildSocialCommentsMarkdown(xiaohongshuApiComments).includes('**接口用户甲**：接口里的一级评论'));
assert.ok(helpers.buildSocialCommentsMarkdown(xiaohongshuApiComments).includes('**回复用户** 回复 **接口用户甲**：接口里的二级回复'));

const cleanedXiaohongshuComments = helpers.mergeSocialCommentTrees([
  [
    {
      id: 'root-clean-1',
      author: '枫啸霄溟',
      ...
      replies: [{
        id: 'reply-clean-1',
        rootId: 'root-clean-1',
        parentId: 'root-clean-1',
        author: '恶魔汤姆',
        replyTo: '枫啸霄溟',
        content: '谁能想到三个月后flomo服务号要关闭了，不会写API又得举家迁徙[捂脸R]',
        time: '1735460744000',
        likes: '5',
      }],
    },
  ...
  [
    { author: '', content: '加载中' },
    { author: '', content: '不如flomo' },
    { author: '匿名用户', content: '不如flomo' },
    {
      author: '',
      content: '这个是我最近才遇到的一个小工具，设置好之后的效果是：你微信发条消息，这条消息会带上时间戳、自动同步到指定的obsidian文档。虽然对我来说，已经习惯ios待办事项作为临时记录的工作流了。#obsidian #知识管理',
    },
    { author: '匿名用户', content: '谁能想到三个月后flomo服务号要关闭了，不会写API又得举家迁徙' },
  ],
]);
const cleanedXiaohongshuMarkdown = helpers.buildSocialCommentsMarkdown(cleanedXiaohongshuComments);
assert.strictEqual((cleanedXiaohongshuMarkdown.match(/不如flomo/g) || []).length, 1);
assert.strictEqual(cleanedXiaohongshuMarkdown.includes('加载中'), false);
assert.strictEqual(cleanedXiaohongshuMarkdown.includes('这个是我最近才遇到的一个小工具'), false);
assert.strictEqual(cleanedXiaohongshuMarkdown.includes('1727358744000'), false);
assert.ok(cleanedXiaohongshuMarkdown.includes('2024-09-26'));
assert.ok(cleanedXiaohongshuMarkdown.includes('  - **恶魔汤姆** 回复 **枫啸霄溟**'));

const xiaohongshuThreadedComments = helpers.extractXiaohongshuCommentsFromApiPayload({
  success: true,
  data: {
    comments: [
      {
      ...
      },
      {
        ...
        sub_comments: [
          {
            id: 'reply-2',
            root_comment_id: 'root-2',
            target_comment_id: 'root-2',
            content: '用户B回复用户2',
      ...
      },
      {
        id: 'reply-3',
        root_comment_id: 'root-1',
        target_comment_id: 'reply-1',
        content: '用户1第二次回复用户A',
        user_info: { nickname: '用户1' },
        target_comment: { user_info: { nickname: '用户A' } },
      },
    ],
  },
});
const xiaohongshuThreadedMarkdown = helpers.buildSocialCommentsMarkdown(xiaohongshuThreadedComments);
const user1Index = xiaohongshuThreadedMarkdown.indexOf('**用户1**：用户1主评论');
const userAIndex = xiaohongshuThreadedMarkdown.indexOf('**用户A** 回复 **用户1**：用户A第一次回复用户1');
const user1SecondReplyIndex = xiaohongshuThreadedMarkdown.indexOf('**用户1** 回复 **用户A**：用户1第二次回复用户A');
const user2Index = xiaohongshuThreadedMarkdown.indexOf('**用户2**：用户2主评论');
assert.ok(user1Index >= 0);
assert.ok(userAIndex > user1Index);
assert.ok(user1SecondReplyIndex > userAIndex);
assert.ok(user2Index > user1SecondReplyIndex);
assert.strictEqual(xiaohongshuThreadedComments.length, 2);
assert.strictEqual(xiaohongshuThreadedComments[0].replies.length, 2);
assert.strictEqual(xiaohongshuThreadedComments[1].replies.length, 1);
...
assert.match(proAiXiaohongshuMarkdown, /^keywords:\n  - AI关键词\n  - 小红书评论/m);
