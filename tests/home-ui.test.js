...
assert.match(wxml, /wx:if="\{\{showNativeTemplateAd\}\}" class="native-template-ad-section"/);
assert.match(wxml, /<ad-custom/);
assert.match(wxml, /unit-id="\{\{nativeTemplateAdUnitId\}\}"/);
assert.match(js, /showAdEntry: false/);
assert.match(js, /showNativeTemplateAd: false/);
assert.match(js, /nativeTemplateAdUnitId: NATIVE_TEMPLATE_AD_UNIT_ID/);
assert.match(js, /INTERSTITIAL_AD_UNIT_ID = 'adunit-080aa7a67f50b9de'/);
assert.match(js, /NATIVE_TEMPLATE_AD_UNIT_ID = 'adunit-a9b616c898835639'/);
assert.match(js, /wx\.createInterstitialAd/);
assert.match(js, /shouldShowGeneralAd/);
...
assert.doesNotMatch(wxml, /wx:if="\{\{currentView === 'collect' && !supportVisible\}\}" class="support-bubble-button"/);
assert.match(wxml, /wx:if="\{\{false\}\}" class="support-bubble-button"/);
assert.match(wxml, /wx:if="\{\{supportVisible\}\}"/);
assert.match(wxml, /bindtap="submitSupportFeedback"/);
assert.match(wxml, /class="support-bubble-button"/);
assert.match(wxml, /class="support-chat-panel"/);
assert.match(wxml, /class="support-message assistant"/);
assert.match(wxml, /class="support-message user"/);
assert.match(wxml, /class="support-message hermes"/);
assert.match(wxml, /\{\{supportLastContent\}\}/);
assert.match(wxml, /\{\{supportReplyText\}\}/);
assert.doesNotMatch(wxml, /support-entry-row/);
assert.doesNotMatch(wxml, /support-entry-button/);
assert.doesNotMatch(wxml, /supportAutoReplyEnabled/);
assert.doesNotMatch(wxml, /toggleSupportAutoReply/);
assert.doesNotMatch(wxml, /switch checked/);
...
assert.match(wxss, /\.native-template-ad-section/);
assert.match(wxss, /\.announcement-sheet/);
assert.match(wxss, /\.support-bubble-button/);
assert.match(wxss, /\.support-chat-panel/);
assert.match(wxss, /\.support-message\.hermes/);
assert.doesNotMatch(wxss, /\.support-entry-button/);
assert.doesNotMatch(wxss, /\.support-ai-row/);
assert.match(wxss, /\.view-mine/);
assert.match(wxss, /\.usage-status/);
assert.match(wxss, /\.upload-progress-card/);

assert.match(js, /DEFAULT_PLUGIN_VERSION = '1\.3\.3'/);
assert.match(js, /ANNOUNCEMENT_VERSION = '2026-07-05-plugin-133-pro-price'/);
assert.match(js, /DEFAULT_PLUGIN_UPDATED_AT = '2026-07-05 08:04'/);
assert.match(js, /Pro 用户支持小红书长文笔记提取图片文字/);
assert.match(js, /飞书文档转存支持接入飞书官方 API/);
...
assert.match(js, /supportVisible: false/);
assert.match(js, /supportCategory: 'question'/);
assert.match(js, /supportLastContent: ''/);
assert.match(js, /supportReplyText: ''/);
assert.match(js, /showSupportPanel/);
assert.match(js, /hideSupportPanel/);
assert.match(js, /submitSupportFeedback/);
assert.match(js, /this\.inboxService\.submitFeedback/);
assert.match(js, /autoReplyEnabled: true/);
assert.match(js, /hermesReply/);
assert.match(js, /supportLastContent: content/);
assert.doesNotMatch(js, /supportAutoReplyEnabled/);
assert.doesNotMatch(js, /toggleSupportAutoReply/);
