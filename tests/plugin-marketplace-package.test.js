...
assert.ok(/本次为\s+`\d+\.\d+\.\d+`/.test(checklist), 'release checklist must declare a target version');
...
assert.ok(
  new RegExp(`PLUGIN_RUNTIME_VERSION\\s*=\\s*["']${manifest.version.replace(/\./g, '\\\.')}["']`).test(pluginMainSource),
  'marketplace main.js runtime identity must match manifest version',
);
...
assert.ok(releaseWorkflow.includes('zip -r "$ZIP_PATH" main.js manifest.json styles.css versions.json README.md LICENSE local-asr local-ocr'));
assert.ok(releaseWorkflow.includes('.artifacts/release-candidate-result.json'));
assert.ok(releaseWorkflow.includes('--verify-promotion release-candidate.json'));
...
assert.ok(macOcrInstaller.includes('tencent-authorized-first-v1'));
assert.ok(macOcrInstaller.indexOf('if install_ocr_packages_from_wheelhouse') < macOcrInstaller.indexOf('Authorized OCR wheelhouse unavailable; trying Tencent PyPI mirror.'));
...
assert.ok(windowsInstaller.includes('$InstallerScriptVersion = "1.2.32"'));
...
assert.ok(windowsInstaller.includes('--retry 0'));
assert.strictEqual(windowsInstaller.includes('Invoke-WebRequest -Uri $Url'), false, 'failed curl downloads must not restart through another transport');
...
assert.ok(windowsInstaller.includes('Complete-InstallerCleanup -TemporaryRoot $TempRoot -Mutex $installMutex'));
assert.match(windowsInstaller, /finally\s*\{\s*try\s*\{\s*Release-InstallLock -Mutex \$Mutex/);
require('./plugin-installer-finalization.test');
assert.ok(windowsInstaller.includes('Prepare-ZipForExtraction -ZipPath $cacheFile -TempRoot $TempRoot -Label $Label'));
assert.ok(windowsInstaller.includes('Download-File -Url $url -OutFile $cacheFile -Resume'));
...
assert.ok(windowsInstaller.includes('--speed-time $DownloadLowSpeedTimeoutSeconds'));
...
assert.ok(windowsInstaller.includes('-C - -o $OutFile'));
...
assert.ok(macInstaller.indexOf('if install_asr_packages_from_wheelhouse') < macInstaller.indexOf('Authorized ASR wheelhouse unavailable; trying Tencent PyPI mirror.'));
