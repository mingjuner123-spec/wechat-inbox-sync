...
function buildTranscriptOnlyMetadata(metadata, {
  ...
  return {
    ...
    commentExtractionStatus: Array.isArray(comments) && comments.length ? 'success' : commentExtractionStatus,
    commentExtractionError,
...
function normalizeSocialComment(comment) {
  ...
  const normalized = {
    id: String(comment.id || comment.commentId || comment.comment_id || '').trim(),
    rootId: String(comment.rootId || comment.root_id || comment.rootCommentId || comment.root_comment_id || '').trim(),
    parentId: String(comment.parentId || comment.parent_id || comment.parentCommentId || comment.parent_comment_id || comment.targetCommentId || comment.target_comment_id || '').trim(),
    author,
    content,
...
function buildSocialCommentsMarkdown(comments = []) {
  const items = threadSocialComments((comments || []).map(normalizeSocialComment).filter(Boolean));
    ...
    try {
      ...
      if (isXiaohongshuUrl(url) || isDouyinUrl(url)) {
        ...
        if (isXiaohongshuUrl(url)) {
          ...
          if (extractedXiaohongshu.comments && extractedXiaohongshu.comments.length) {
            extractedXiaohongshu.commentExtractionStatus = 'success';
        ...
        if (mediaUrl) {
          return await this.buildTranscriptRecordFromMedia(record, {
            ...
            binding,
            title,
            comments: isXiaohongshuUrl(url) && extractedXiaohongshu ? (extractedXiaohongshu.comments || []) : [],
            commentExtractionStatus: isXiaohongshuUrl(url) && extractedXiaohongshu
              ? (extractedXiaohongshu.commentExtractionStatus || ((extractedXiaohongshu.comments || []).length ? 'success' : 'empty'))
              : '',
            commentExtractionError: isXiaohongshuUrl(url) && extractedXiaohongshu ? (extractedXiaohongshu.commentExtractionError || '') : '',
