...
} = require('../quickstartFunctions/admin-core');
const {
  DEFAULT_REDEEM_PLAN,
  normalizeRedeemCode,
} = require('../quickstartFunctions/redeem-code-core');
const {
  createBindStatusResponse,
} = require('../quickstartFunctions/inbox-core');
const {
...
async function updateEntitlement(request) {
  ...
  const updateData = { updatedAt: now };
  if (action === 'extend') {
    const days = normalizeAdminPositiveInteger(request.body.days, 1, 3650, 30);
