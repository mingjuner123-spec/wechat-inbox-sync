...
### 2026-08-28 - 1.3.130 ASR 自动回收与小红书隐藏浏览器联合候选

- 以公开 `main@2fd500b83ffc4f20d7d7ff881d0c68c67c9fa6b1`（1.3.129）为唯一基线，合并 ASR 安装进程自动回收与小红书隐藏窗口/Cookie 门禁/脱敏诊断；未改 CloudBase、小程序、权益、支付或用户数据。
- 从合并后的 `src` 统一生成正式 Bundle，根目录四项 loose assets 与插件目录一致。
- 独立审查发现初版仅用子串判断 `-InstallRoot`，邻近目录可能被误认成同一 ASR 安装目录；进一步复核又发现脚本名不能只在整条命令行中搜索。现已分别解析 PowerShell 的 `-File` 与 `-InstallRoot`，只接受正式安装器 basename 和规范化绝对目录完整等值；新增路径前后缀、空格、单双引号、等号形式以及 `-Command` 仅提及脚本名不得命中的回归。
- 已验证修正版候选：`1.3.130-13dde82a5a9db0f3`，aggregate SHA-256 `13dde82a5a9db0f30b660c561f5be833b33e3adc007543b0b24aa92ff490ad9f`。
- 本地正式 CI 门禁与本次专项回归共 37 个测试文件通过；组件 manifest、私有访问策略、语法、候选安装/回读/晋升均通过。发布仍须完成修正版联合差异复核、远端版本空闲复核、受保护 main PR、tag/Release 与发布后逐层验收。

### 2026-08-27 - 修复小红书自动窗口外露、无 Cookie 评论访问与诊断缺失（本地候选）

- 目标：自动小红书登录探测、正文、媒体和评论窗口持续隐藏；无 `web_session` 时评论路径零页面访问；诊断记录 Cookie 有无、访问阶段、拦窗事件和 `300011`，不保存 Cookie 值、正文或完整 URL。
- 实现：兼容 Electron `did-create-window(window, details)` 并销毁子窗口，对 `ready-to-show`/`show` 重复隐藏；登录探测和评论渲染边界均先检查 Cookie，缺失时记录 `skipped_no_login_cookie` 并直接返回；成功或失败同步均可保留脱敏浏览器诊断。显式登录窗口仍可见，正文提取仍允许访问用户提交的小红书内容页。
- 非目标：未增加 `300011` 熔断、额外重试、评论开关或清除登录态；未修改版本号。
- 验证：主插件、小红书登录窗口、同步诊断、市场包、源码/产物语法、构建一致性和 diff 检查通过；独立审查提出的评论函数边界 P2 已修复并复核闭环，无新阻断。
- 外部动作：无。未提交、未推送、未发布、未替换本机插件，未访问真实用户数据、Cookie 内容或线上环境。
- 已知风险：正文提取仍可能触发平台风控；Cookie 存在但已失效时仍需隐藏登录探测页面确认，这两点不属于本次消除的无 Cookie 评论访问。
...
- 历史对照：`1.3.30` 先取精确详情，失败后直接使用当前页面可播放媒体；后续版本逐步加入唯一身份、混合身份、多个无身份播放器和无稳定 aweme ID 的失败关闭，造成不同电脑因精确接口可用性不同而出现“有人成功、有人始终失败”。
- 本地实现：继续优先使用精确详情/目标响应；浏览器兜底改为按目标身份加分、播放状态、可见性、面积和 DOM 顺序选择当前主媒体，不再因候选/页面身份混杂或缺失而拒绝；无稳定 ID 也进入非严格浏览器兜底。仅浏览器最终地址明确跳到另一个具体作品时拒绝。
- 长期规则：诊断标签不得演变成产品阻断；任何新提取阻断都要有真实事故、脱敏复现样本和历史可用样本对照。
- 外部动作：无。版本仍为 `1.3.90`，未替换用户插件、未发布、未部署、未修改 CloudBase、绑定码、权益、支付或用户数据。

### 2026-08-15 - 修复飞书附件已保存但 Markdown 不展示，并收紧抖音目标媒体兜底

- 根因：飞书官方 API 链路先把稳定图片 token 改成临时 URL，附件写入后又依赖 URL 字面匹配替换 Markdown；HTML 转义或临时地址差异会造成附件已落盘但正文仍保留坏链接，并被错误计为成功。抖音补充回归则来自页面加载目标作品后跳到通用首页，最终 URL 丢失作品 ID。
- 实现：飞书以 `feishu-image:{token}` 贯穿 Markdown 与 `/feishu/media` 下载，只对未解决 token 使用临时地址兜底；图片 URL 比较先做 HTML 实体规范化，且只有 Markdown 实际替换成功才计为本地化。抖音允许精确目标 DOM 播放器和唯一页面目标下的单一可见播放候选，多个无身份候选、冲突身份和推荐流继续拒绝。
- 验证：飞书、抖音、媒体诊断、完整插件、社媒上下文、源码与 bundle 语法、构建一致性及 diff 检查通过；独立复审 P0/P1/P2 均为 0。
- 外部动作：无。当前为隔离分支本地测试候选，未替换已安装插件、未发布、未部署、未修改绑定或用户数据。

### 2026-08-15 - 修复抖音作品真实媒体提取并准备 1.3.87 发布

- 目标：修复部分用户抖音短链页面能打开、ASR 可用，但插件无法取得目标作品真实音视频地址而转写失败的问题。
- 根因：抖音作品详情接口近期经常只返回页面壳；1.3.86 隐藏浏览器仅接受调试协议中的精确 JSON 媒体，遗漏了已由最终 URL 或 canonical URL 证明属于目标作品的主播放器 DOM 媒体。
- 实现：保留精确详情媒体最高优先级；新增 final/canonical 作品身份约束下的主播放器兜底，按目标身份、播放状态、可见性和面积选择；明确冲突、缺少作品身份和推荐流候选继续拒绝；不再主动播放页面全部视频；新增 modal_id 识别及 403/过期媒体的同身份刷新路径。
- 候选：版本 1.3.87，Candidate ID 1.3.87-4a3716f5501a684b，聚合 SHA-256 4a3716f5501a684bb8ec477f04086de54f8faea04cf9c3a2d78066ef87d42f23。
- 验证：发布流水线 32/32 组通过；发布治理 126/126、发布身份 26/26；源码与 bundle 语法、构建、根镜像、候选晋升、社媒上下文、市场包及 diff 检查通过。独立复审 P0=0、P1=0、P2=0。
- 外部动作：负责人已授权公开发布；当前记录为发布前检查点，未修改 CloudBase、业务数据、绑定、支付或本机 data.json。

### 2026-08-09 - 回调小红书隐藏窗口能力并修复暂停未真正终止

- 目标：解决保持小红书登录态提取评论时批量弹出浏览器，以及“暂停当前转写”只更新界面但底层提取、评论或排队任务继续运行的问题。
- 历史定位：2026-07-27 的 `21a0808b` / 1.3.69 开始允许隐藏提取窗口创建同域子窗口；2026-08-07 的 `f6dd0d5b` / 1.3.77 将评论总量提高到 1000 后放大页面活动。1.3.78 虽恢复 300 条上限，仍保留子窗口能力；暂停控制器也没有传入小红书浏览器和评论循环。
- 本地实现：以 1.3.78 为底座做模块级回调，不整包回退；隐藏提取/评论窗口统一禁止 `window.open` 子窗口，保持 300 条和 90 秒预算；每条记录的取消信号贯穿登录探测、隐藏浏览器、评论循环、媒体候选和写入链路；修复排队任务取消后可能卡死浏览器锁的问题。候选提交为 `412b8dc6`。
- 验证：小红书登录窗口、处理取消、重复内容、本地去重、社媒转写上下文、标题/转写质量、插件构建和市场包定向回归通过；完整 `plugin-main-ai` 仍停在本次修改前已存在的外站跳转错误码断言矛盾，基线 `2114ac81` 已确认同样存在，未把它误归因于本次修复。
- 外部动作与授权：负责人明确要求直接退出 Obsidian 并安全替换。本机 6 个 Obsidian 进程退出后，只替换 `main.js`、`manifest.json`、`styles.css`、`versions.json`；未发布、未部署、未修改 CloudBase 或云端记录。
- 配置保护与回退：替换前后 `data.json` SHA-256、绑定数量（1）和绑定令牌脱敏指纹完全一致；目标程序文件与候选匹配。回退目录为 `.obsidian/plugin-backups/wechat-inbox-sync-before-xhs-stop-20260809-151207`，候选 staging 为 `.obsidian/plugin-staging/wechat-inbox-sync-xhs-stop-20260809-151207`。
- 结果：本机测试插件保持版本身份 1.3.78，并已换入小红书弹窗与暂停根因修复；等待负责人重新启动 Obsidian 做真实登录态评论和暂停测试，未进行插件市场发布。

### 2026-07-31 - 插件发布候选管线 V2 已进入远端默认分支

- 目标：先解决发布过程的四类系统问题——发布源/测试源漂移、本地已测包与最终 Release 不一致、Windows 本地替换失败破坏可用版本，以及 Release 成功后远端标签正确却被 Runner 本地状态误报失败。
- 影响范围：发布脚本、GitHub 工作流、候选包凭据、发布清单、测试与治理文档；未修改插件业务逻辑、ASR/OCR、版本号、小程序、云函数或真实用户数据。
- 实现：以 `obsidian-plugin/wechat-inbox-sync/` 为唯一源码生成不可变完整候选；根目录四镜像只能单向同步；本地安装以验证成功为提交点，提交前失败逐项回滚并保留无法恢复的备份，提交后备份清理失败不再撤销已验证新版本，同时保留 `data.json`；提交内 `release-candidate.json` 绑定已测候选；CI 与 Release 只消费该候选；prepublish/postpublish 都从受信 commit 重算候选凭据，postpublish 再通过 GitHub API 与远端 Git 引用多次回读 annotated tag 和资产身份。
- 候选身份：版本 `1.3.74`，Candidate ID `1.3.74-3b2e96b51110bb77`，完整包聚合 SHA-256 `3b2e96b51110bb77ead1d855f51148d0f8e330459e8d09fe9730e5b481235532`，14 个受控文件。
- 验证：候选管线 24 通过、0 失败、1 个普通 symlink 用例因 Windows `EPERM` 跳过（等价 junction 防护通过）；发布身份 26/26、发布治理 126/126、插件核心、市场包、脚本语法、镜像精确校验、候选凭据校验、业务源码零差异和 `git diff --check` 全部通过。独立插件仓库未包含两项 Harness 检查脚本，因此未伪报执行。
- 审查：候选身份/晋升、事务安装与回滚、CI 绑定、固定时钟、远端 tag 身份、Release 候选资产及整分支均交由独立子 Agent 审查；发现的问题已修复并复审，最终结论以任务卡回执为准。
- 外部动作：负责人明确授权“发布逻辑更新”后，分支 `codex/plugin-release-pipeline-v2` 已推送；PR [#41](https://github.com/mingjuner123-spec/wechat-inbox-sync/pull/41) 的 `guards` 与 `windows-deployer` 两道检查通过，并以 squash 方式合并到 `main@910178090dcdb899525e8bab1c8fb3994561c69b`。未打标签、未创建或覆盖 Release、未发布插件、未部署 CloudBase。
- 回读与回退：远端 `main` 已回读为 `910178090dcdb899525e8bab1c8fb3994561c69b`，最新正式标签仍为 `1.3.74`；如需回退，只对该 squash 提交执行普通 revert，不移动历史标签、不覆盖 Release。
- 结果：发布流程已由默认分支强制执行候选身份、根镜像一致性、Windows 事务安装和 Release 资产校验；本地架构候选仍独立存在，未进入远端、未替换用户插件。
- 下一步：把已完成终审的本地架构候选事务安装到负责人测试库，完成核心功能人工验收；只有人工验收通过后才另开 L3 发布任务。

### 2026-07-30 - 修复失效短链跨环境假删除并清理历史重复记录

- 目标：让失效小红书短链真正退出短业务环境队列，不再因长环境幂等成功或历史重复记录逐条浮出而反复同步。
- 根因：插件把显式删除路由固定到长环境，但真实 `inbox_records` 位于短业务环境；长环境返回 `deleted` 不能证明短环境记录已删除。同时同步 API 会按链接去重，每次只暴露同链接最新一条，导致删除一条后更旧重复项继续出现。
- 真实动作：按用户授权和两道独立复审，固定短业务环境、逐轮精确匹配同一失效 URL、`pending`、`webpage` 且无云文件的记录，通过现有 `/records/:id/synced` 物理删除契约额外清理 3 条历史重复记录；点时回读可见 pending 总数和目标数均为 0。
- 本地修复：暂停转写删除和失效短链删除统一改走当前短业务 `apiBase` 的 `/records/:id/synced`；飞书 OAuth 仍独立走长环境。同步登录探测、正文看门狗、并发锁、失效说明和图片去重保持不变。
- 验证：`plugin-main-ai`、`plugin-marketplace-package`、插件源和安装文件语法、`git diff --check` 通过；本地安装文件与候选源 SHA-256 一致。未发布、未部署。

### 2026-07-27 - 恢复 1.3.30 小红书公开图文语义（1.3.71 候选）

- 目标：修复 1.3.68～1.3.70 在网络和隐藏浏览器已取得正文、标签及真实图片后，仍因通用标题、缺少精确 noteId 或候选身份标签把公开图文判定失败的问题。
- 根因：1.3.52 开始的通用页拒绝和 1.3.69/1.3.70 的精确身份门槛覆盖了 1.3.30 的内容可读判定；新版多页面候选又按内容量选最高分，可能让更丰富的推荐页覆盖原始短链结果。
- 实现：恢复 1.3.30 的公开内容验收——可信小红书页面有真实图片即可，无图片时正文至少 20 字；不再因通用标题、缺少 noteId、候选或推荐标签直接拒绝。纯分享口令和明确失效页仍保持待重试。多个浏览器结果按原始短链第一个可读结果优先，后续只有精确身份才能覆盖；仅相同非通用标题或共享真实图片的快照允许互补字段。
- 保留能力：免费公开图文、Pro 音视频转写与文字图片 OCR、评论仅 Pro＋已登录、AI 429 不阻断正文、本地 ASR/OCR 向后兼容、暂停转写删除均未回退；未修改小程序、CloudBase、绑定、权益、支付或真实用户数据。
- TDD：先证明 1.3.70 会拒绝“通用标题但已有完整正文和两张真实图片”；又证明内容更多的推荐页会错误覆盖原始短链。修复后新增/更新回归覆盖公开图文、纯分享空页、跨页不乱拼、同笔记互补快照、音视频媒体继续探测及评论权限矩阵。
- 验证：`plugin-main-ai`、`plugin-marketplace-package`、插件语法、`git diff --check`、`release-governance`（126/126）、`plugin-release-identity`（23/23）、组件 manifest 检查和公网 CDN 完整性检查全部通过；独立审查 P0/P1/P2 均为 0。
- 发布状态：候选版本身份已统一为 1.3.71，正式发布仍须通过受保护分支和不可变 Release 流程；1.3.70 为回退锚点。
- 复盘：形成长期决策“无真实事故证据和历史对照回归，不得给核心提取链路新增失败阻断规则”，避免诊断标签再次演变为过度防护。

### 2026-07-25 - 恢复剪切板链接自动提取并统一 Windows 保存路径（1.3.59）

- 目标：恢复旧客户端或“读取剪切板”入口中单一公开 HTTP(S) 链接的网页提取能力，同时消除 `raw\wechatmd` 与 `raw/wechatmd` 在 Windows/Obsidian 中形成同一物理目录双别名的风险。
- 影响范围：仅 Obsidian 插件正式发布源、根目录 Marketplace 版本元数据、插件回归/发布身份测试、任务卡、设计/实施文档和本工作日志；不修改小程序、云函数、Pro/绑定规则、业务数据或用户知识库现有文件。
- 实现：`text` 记录中的单一安全链接会在内存中提升为网页提取并把完整原始剪切板文案写入笔记；纯文本、含糊多链接、非 HTTP(S)、带凭据、本机/私网/保留地址保持文本。自动网页提取失败会明确失败并保持待重试，不写伪成功笔记、不调用 `/synced`。保存目录设置和所有最终 adapter 写入路径统一使用 `/`；已有目录直接复用，缺失目录逐级创建，并容忍并发 exists/create 竞态。
- 安全纠偏：独立安全复审先后阻止了字面 URL 校验不足、DNS/重定向 SSRF、失败假成功、`shareText` 未落盘、目录创建竞态、跨域认证头泄漏和特殊用途 IP 漏判。最终实现使用实际连接的公网 DNS lookup、逐跳重定向校验和响应大小/超时边界；跨域或 HTTPS 降级只允许 `Accept`、`Accept-Language`、`User-Agent`，同源 HTTPS 跳转才保留原认证头。
- 验证：TDD 覆盖无空格中文分享文案、通用网页、小红书、含糊多链接、危险 URL、DNS 混合集、私网跳转、跨域敏感头、失败不确认同步、原始文案正文留存，以及日期目录、视频号、语音、文件、图片、原始媒体附件和并发建目录路径矩阵。完整本地门禁包括 `plugin-main-ai`、`plugin-marketplace-package`、`release-governance`（126/126）、`plugin-release-identity`（23/23）、插件语法、组件清单和 `git diff --check`。
- 发布约束：目标版本为 `1.3.59`，只允许经受保护默认分支门禁合并后创建不可变 annotated tag；正式完成必须同时满足 main/tag/Release 提交一致、五项 Release 资产、Raw manifest、Release ZIP 与本地下载 ZIP 一致。线上执行结果由本任务最终回执和飞书完成通知确认。
- 数据变更：无；不会移动、复制或删除用户已有文件。
- 已知风险：插件兼容可覆盖现有和排队中的旧 `text` 记录，但小程序与云端链接类型合同仍需在干净基线上另立任务统一；私密、删除、过期或被平台风控的页面仍会按真实提取失败保留重试。
...
- 线上动作：PR `#13` 通过 `guards` 与 `windows-deployer` 后合并为 `cbf2b76b6a63848f53ba2b7e44d55ffe4afd0305`；受控本地组件部署器已将 Windows OCR 安装器发布到 SHA-256 `58987dde9d22e97428eb3fe2a6705b702856e6450e308a7659b7f311d27b894e` 的不可变对象，随后切换兼容别名并发布公网组件清单，完整 CDN 校验通过。本次不需要升级 Obsidian 插件版本。
- 数据变更：无。
...
- 下一步：让受影响用户在插件设置中再次点击一次“安装/修复本地转写组件”；若仍失败，复制新的诊断信息继续定位安全软件或系统组件问题。
