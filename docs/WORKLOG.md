...
### 2026-07-30 - 完成插件发布候选管线 V2（本地，待集成）
...
- 外部动作：无。分支 `codex/plugin-release-pipeline-v2` 仅存在于隔离 Worktree；未推送、未合并、未打标签、未创建或覆盖 Release、未部署。
- 已知风险：该管线进入远端默认分支前，线上仓库仍使用旧发布流程；根目录镜像仍保留只是为了 Obsidian 市场兼容，但已经由自动校验阻止人工漂移。
- 下一步：由负责人决定集成方式；集成完成后回到主线，系统阅读和拆分插件 `main.js`，先产出架构地图再做业务重构。
