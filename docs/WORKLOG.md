...
- Online action: merged through PR #2 after the required `guards` check passed. The controlled deployer published the installer, compatibility metadata, immutable compatibility archive, and `local-asr/windows/whisper-bin-x64-compat.zip` alias. CloudBase download and cache-bypassed public CDN download both returned SHA-256 `7B562DEEF031BD8A1A3954E3F5FF43BE0ACE2E86974235518530594BEECFF4B7`; the public installer contains version `1.2.23` and the fallback archive URL.
- Data changes: none.
...
- Next: ask the affected user to click “安装/更新本地转写组件” again and return the new diagnostic only if the compatibility package also fails. Failure-history logging is explicitly deferred.
