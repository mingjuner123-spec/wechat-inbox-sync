...
- 对已经稳定工作的公开内容提取能力，新增安全或质量规则默认只能记录诊断、降低增强能力或进入备用路径，不得直接把已有可用内容改判为失败。
- 任何新的失败关闭条件必须同时具备：真实事故或明确威胁证据、可复现失败样本、历史可用样本的对照回归，以及对免费/Pro、跨平台和旧客户端兼容性的影响说明。
- 修复第三方网页变化时优先恢复最后可用版本的简单语义，再按真实失败逐层增加兼容；禁止用“唯一候选、必须精确身份、推荐流一律拒绝”等推测性规则替代实际内容验收。
- 多个备用页面可以用于跨设备容错，但选择顺序必须确定：原始用户入口的第一个可读结果优先；后续结果只有取得更强的同一内容证据时才能覆盖。不同页面不得仅因正文或图片更多而相互拼接或抢占。
- 纯分享口令、明确失效页、非官方域名和危险协议仍可拒绝；这些属于可复现的空内容或网络边界，不得扩展成对通用标题、缺少 noteId 或推荐标签的普遍拒绝。

原因：

- 1.3.52 至 1.3.70 逐步加入通用页拒绝、精确 noteId 和候选身份门槛后，真实页面已经返回正文和数十张图片仍被判定失败，破坏了 1.3.30 的历史可用能力。
- 过度防护没有解决网络或平台变化，反而把诊断标签变成产品阻断条件，并在多个浏览器候选架构下产生推荐页抢占和跨页拼接等新的复杂性。
- 以真实内容是否存在作为成功标准、以确定顺序处理备用通道，可以同时保持跨设备容错和可理解性。

### 2026-08-16：该规则对抖音媒体提取的具体解释

- 未查到“用户提交作品 A，插件实际转写推荐作品 B”的真实线上事故；历史中的“推荐流风险”来自开发推测与合成测试，不能继续作为失败关闭依据。
- 精确作品详情、目标响应和已认证会话仍优先，但它们失败后必须恢复当前页面主播放器兜底；推荐标签、混合身份、缺少唯一身份、多个无身份播放器和缺少稳定 aweme ID 只能进入诊断与排序，不能单独阻断转写。
- 只保留可复现的明确错页边界：浏览器最终地址包含另一个具体作品 ID。canonical、页面预加载身份和播放器祖先节点的混合 ID 不足以证明错页。
- 以后新增第三方平台提取阻断，必须先提供真实事故记录和会在当前稳定版失败的脱敏回归样本；“担心可能误抓”本身不构成上线理由。
...
## 2026-07-27：插件升级不得仅因脚本版本较旧而停用已安装 ASR

决策：

- 安装器“是否允许执行”与已安装转写脚本“是否还能继续运行”是两个不同判断。下载到的安装器继续执行严格新鲜度校验；已经安装且具备当前最低安全/质量能力的旧转写脚本，即使缺少最新诊断增强，也必须按兼容模式继续运行。
- ASR 脚本状态统一分为“当前”“已知兼容、建议升级”和“明确不兼容”。只有脚本缺失、关键质量门缺失、包含已退役提示词参数、损坏，或已有回归证据证明不能安全运行时，才能令 `ready: false`。
- “建议升级”不得自动进入网络修复路径。CDN 返回 HTTP 418、超时、离线或随包安装器缺失时，插件必须保留并继续使用原有可运行脚本。
- Windows `transcribe.ps1` 更新采用事务切换：先生成并验证候选文件，保留原脚本备份，完成组件验证后才提升候选；后续任一步失败恢复备份，全部成功后才清理短期备份。
- 每次提高本地组件能力标记，发布门禁必须覆盖至少上一受支持脚本，并模拟安装器成功、HTTP 418、超时和本地备用缺失；不得只测试全新安装或当前脚本。

原因：

- `1.3.57` 为修复 Windows 原生进程输出读取问题引入 `diagnostics-process-v1`，但错误地把仍具备重复幻觉防护、心跳进度、UTF-8、安全模型回退和崩溃诊断的上一版 `Start-Process` 脚本直接判为不可用。
- 用户的 Whisper、FFmpeg、模型和旧转写脚本并未损坏；插件更新后的强制验收与一次 CDN HTTP 418 叠加，才把原本可运行的安装变成同步失败。版本升级不应把网络可用性变成继续使用本地组件的前置条件。
