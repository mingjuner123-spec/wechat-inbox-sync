...
候选目录位于已忽略的 `.artifacts/`，不进入 Git。它是测试过程中的冻结物，不是长期发布仓库。正式历史仍以 Git commit、annotated tag 和 GitHub Release 为准。

## 内容身份证

`candidate.json` 只记录可公开、可复核的信息：
...
- 创建时的 Git HEAD；
- 权威源码相对路径；
- 四个发布文件的 SHA-256、字节数和相对路径；
- 整组内容的聚合 SHA-256。

候选 ID 由插件版本和聚合哈希前缀组成，不用时间戳充当身份。文本文件在生成候选时统一为 LF；因此 Windows 与 Linux 对同一内容得到相同候选字节。候选生成后若目标目录已存在，只有逐字节完全一致才允许复用，否则拒绝覆盖。
...
- SHA-256 和聚合身份；
- 候选身份证构建与验证；
- 源码、候选包、根镜像和安装目标的文件集合比较。

它不执行 Git、网络或文件写入，便于用真实临时目录做单元测试。

### `scripts/prepare-plugin-release-candidate.js`

从权威源码读取四个发布文件，验证 manifest 版本一致后，在 `.artifacts/plugin/<candidate-id>/` 原子生成候选。
...
安装前确认目标目录末级名称为 `wechat-inbox-sync`，并确认候选身份有效。只替换四个发布文件，保留现有 `data.json` 和其他用户设置。安装使用临时文件与逐文件替换，完成后立即回读哈希；任何失败都返回非零并指出失败文件。

脚本不得猜测知识库路径，不得扫描用户目录，也不得读取 `data.json` 内容。
...
验证任一对象有新增、缺失、哈希或版本漂移即失败。它不自动修复，不自动复制，也不发布。
...
3. 用户进行真实功能测试。
4. 用户确认通过后，再次运行候选验证。
5. 只有源码、候选、根镜像和安装目标仍一致时，才允许进入提交、PR、合并与标签流程。
6. 任一其他任务改写源码后，旧候选验证失败；必须重新生成候选并重新测试，不能把旧测试结论沿用到新内容。

本任务不自动执行第 5 步中的外部动作，只建立可供后续发布任务调用的失败关闭门禁。
...
- GitHub Git Ref API 返回标签引用；
- GitHub Git Tag API 证明对象类型为 annotated tag，并指向正确 commit；
- `git ls-remote` 的标签与 peeled commit 一致；
- Release target、五项资产和 ZIP 内容一致。

postpublish 不再要求 Runner 本地 `refs/tags/<version>` 必须是 annotated 对象。Runner 可以处于 detached checkout，或本地只存在工作流生成的轻量引用；只要远端权威证据完整就通过。它仍检查本地 HEAD 与发布资产内容，不能用此变化绕过版本身份校验。
...
`release.yml` 保持“先完整门禁、后创建 Release、最后回读”的顺序。最后一步调用远端权威 postpublish 模式，不再依赖 Runner 本地标签对象类型。
...
- 候选目录已存在但内容不同：拒绝覆盖。
- 权威源码包含未声明发布文件：不自动加入；仍只处理四项白名单文件。
- manifest/versions 版本不一致：候选生成失败。
- 安装目标不是明确的 `wechat-inbox-sync` 目录：安装失败。
- 安装中断：不删除 `data.json`；保留原文件或明确报告部分替换，不宣称成功。
- 测试后源码或根镜像改变：验证失败并要求重新生成、重新测试。
...
- 候选只包含四个发布文件。
- 候选目录不可用不同内容覆盖。
- 源码、根镜像、候选包和安装目标一致时通过。
- 修改、删除或新增受控文件后失败。
- 安装保留 `data.json`，且不读取其内容。
- 错误目标目录被拒绝。
...
- 远端 lightweight tag、tag 对象指向错误 commit、Release target 漂移或资产漂移仍失败。
...
1. 不存在插件业务文件的行为差异。
2. 可以从权威源码生成一个不可变候选并安装到指定测试目录。
3. 用户设置文件不被候选安装覆盖。
4. 测试完成后任意一份内容被改动，候选验证立即失败。
