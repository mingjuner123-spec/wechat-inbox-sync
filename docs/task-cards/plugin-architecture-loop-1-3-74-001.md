...
- 负责人关闭 Obsidian 后，已通过事务安装器把架构候选安装到测试知识库。安装前后 `data.json` SHA-256 均为 `62E9DA8EA43A0A9D3198669CB3CAFD0B1DB41D73EEFA314EC33A88F505E5C2FB`，长度均为 2817 字节。
- 安装后候选身份证、完整受控包、已安装目录和四项 loose assets 复验通过；已安装 `main.js` SHA-256 为 `08E4B7AEB9C6CFCCC34D12231A2D7EA4F52A801E8F382AEF725445FFC9796728`，语法检查通过，版本仍为 `1.3.74`。
- 安装事务没有遗留 `.stage-*` 或 `.backup-*` 目录；当前进入负责人核心功能人工测试。测试通过前仍不得晋升正式候选、推送架构分支或发布插件市场版本。
