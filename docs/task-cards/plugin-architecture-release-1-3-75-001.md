...
- 目标发布版本只允许将版本身份递增到 `1.3.75`；最终 `main.js` 只允许内嵌运行版本常量从 `1.3.74` 变为 `1.3.75`。将该唯一版本字符串归一化回 `1.3.74` 后，SHA-256 必须继续精确等于上述人工测试候选。
...
6. 验证 1.3.75 的 `main.js` 仅有唯一运行版本字符串变化，归一化后与人工测试候选逐字节一致，再完成完整回归、独立审查、PR 和合并。
...
- 1.3.75 的 `main.js` 仅有唯一运行版本字符串变化；归一化回 `1.3.74` 后 SHA-256 精确等于人工测试候选。
